...
struct AboutView: View {
    ...
    private let gettingStartedRight: [(title: String, detail: String)] = [
        ...
        ("Crop an image", "Double-click it, or select it and click the toolbar's Crop button (or Edit > Crop), to open the crop tool. \"Apply\" saves the crop on the canvas without touching the file. \"Apply & Export\" also writes a cropped copy into the canvas folder, named with a \"-crop\" suffix. \"Apply & Replace\" instead switches the card to a new, permanently-cropped file and removes the original from the canvas — it stays on disk but won't come back on its own; drag it in from Finder if you want it again."),
    ...
    private let detailsTips: [(title: String, detail: String)] = [
        ...
        ("Art boards", "A canvas can hold multiple art boards, stacked vertically, each with its own images and layout. Click the \"+\" below the last board to add another. Right-click a board's background for \"Move to…\" (move it to a new position) or \"Delete Art Board\" (files stay in the folder, same as Remove from Canvas). Drag a card past a board's edge and it lands on whichever board it's now over."),
        ("Board size", "Use the toolbar's Board Size… button (or File > Set Art Board Size…) to choose Auto — each board sizes itself to fit its own content — or Fixed, where every board shares one size (inches, cm, or pixels, with presets like US Letter, A4, Instagram Post, and HD) and never resizes itself."),
        ("Board colour", "Click the palette icon in the toolbar to set the default background colour for boards that don't have their own. Right-click a board's background and choose \"Board Colour…\" to set just that one board — it's remembered per folder and used when exporting too."),
        ("Text fields & text boxes", "Add one with the toolbar's Add Text Field / Add Text Box buttons (also in the Edit menu, at the bottom). Double-click a text item to type directly into it on the canvas, styled live — a floating panel opens alongside it for typeface, size, Bold/Italic, colour, alignment, letter spacing, and line height. A field doesn't wrap (only an explicit Return breaks a line); a box wraps to its width like a paragraph. Click elsewhere, or press Escape, to finish editing. Drag to move or resize it freely, like reshaping a box — hold ⌘ or ⌥ while resizing instead to scale the text proportionally, font size included."),
        ...
        ("Large preview background", "Use View > Large Preview Background Settings… to set the colour and opacity behind a large image preview, plus whether its filename label shows and what colour it is."),
        ("Guides", "Drag in from the top or left edge to place a guide line. Click one to select it (Delete removes it), or drag it to reposition — drag it back to the edge to remove it. Images snap to guides (and to their board's own edges) when moved or resized nearby. Toggle the \"Guides\" toolbar button (or View > Toggle Guides) to hide them and their ruler strips; change their colour with View > Change Guide Colour, or remove them all at once with View > Clear All Guides."),
        ("Smart guides", "While dragging an image or text item (or a selection of several, moved as one), pink dashed lines appear when an edge or centre lines up with another item on the same board, and green ones when it lines up with the board's own horizontal or vertical centre — both snap the drag into place, like PowerPoint or Keynote's alignment guides. Small double-headed arrows appear too when the gap either side of it matches — evenly spacing it between two neighbours (the board's own edge counts as a neighbour here too, so an item can space itself evenly between the board margin and the next item), or continuing an already-even row or column — and starting a new row below an evenly-spaced one matches the gap between the two too, so the vertical gap ends up the same as the horizontal one. Resizing a single image the normal way (no ⌥) works the same way against other items' edges, so it's easy to match another card's edge or size. Turn this off with View > Turn Smart Guides Off."),
        ...
        ("Create Grid", "Select two or more images and click the toolbar's Create Grid button (or View > Create Grid…) to arrange them into a clean grid. Set the spacing between images in points or as a percentage. By default, images already in a rough row stay together and rows only merge or split where the canvas width requires it, with every image resized to a shared height. Switch to \"Max Per Row\" or \"Max Per Column\" instead to force an exact count along one axis — the other axis simply grows to fit, and a count that doesn't divide evenly just leaves a shorter last row or column."),
        ...
        ("Overset text", "A text field or text box whose content no longer fits its frame shows a small red \"…\" badge over its bottom-right corner."),
    ]
    ...
    private let exportingTips: [(title: String, detail: String)] = [
        ...
        ("Export as PDF", "Use File > Export as PDF… to choose which art boards to include and a resolution (dpi); each becomes one page, in order."),
