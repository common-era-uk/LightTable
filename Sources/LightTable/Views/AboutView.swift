...
struct AboutView: View {
    ...
    private let detailsTips: [(title: String, detail: String)] = [
        ...
        ("Smart guides", "While dragging an image or text item (or a selection of several, moved as one), pink dashed lines appear when an edge or centre lines up with another item on the same board, and green ones when it lines up with the board's own horizontal or vertical centre — both snap the drag into place, like PowerPoint or Keynote's alignment guides. Resizing a single image the normal way (no ⌥) works the same way against other items, so it's easy to match another card's edge or size. Turn this off with View > Turn Smart Guides Off."),
